...
//%endif
		...
		if (gameInput.MapOverlay.Pressed && m_widgetRoots.length() == 0)
		{
			m_showMapOverlay = !m_showMapOverlay;
			Tutorial::RegisterAction("map_overlay");
		}

		if (gameInput.Ping.Down && gameInput.Attack1.Pressed && m_mice !is null && m_mice.length() > 0)
		{
	...
	void UpdateWidgets(int ms, GameInput& gameInput, MenuInput& menuInput) override
	{
		...
		{
			if (gameInput.PlayerMenu.Pressed && (m_widgetRoots.length() == 0 || m_playerMenu.m_visible)) 
			{
...
%if TARGET_XB1
			...
			vec2 overscanMove = (screenCenter - vec2(ret.x + minimapSize/2, ret.y + minimapSize/2));
			overscanMove += vec2(minimapSize*sign(overscanMove.x)/2, minimapSize*sign(overscanMove.y)/2);
			...
			ret.x += overscanMove.x;
			ret.y += overscanMove.y;
%endif
			return ret;
		}
	}
	...
	void RenderWidgets(PlayerRecord@ player, int idt, SpriteBatch& sb) override
	{
		...
		if (!m_playerMenu.m_visible && m_currInput !is null && ShouldShowMinimap())
		{
			...
			vec2 textpos = vec2(m_wndWidth - m_textMapLevel.GetWidth() - 2, -1);
%if TARGET_XB1
			vec2 screenCenter = vec2(g_gameMode.m_wndWidth/2, g_gameMode.m_wndHeight/2);
			vec2 overscanMove = (screenCenter - vec2(textpos.x + m_textMapLevel.GetWidth()/2, textpos.y + m_textMapLevel.GetHeight()/2));
			overscanMove += vec2(m_textMapLevel.GetWidth()*sign(overscanMove.x)/2, m_textMapLevel.GetHeight()*sign(overscanMove.y)/2);
			float overscan = GetVarFloat("ui_overscan");
			if(abs(overscanMove.x) > screenCenter.x*(1 - overscan))
			{
				overscanMove.x *= overscan;
...
%endif
	...
	bool skippedLastMinimapUpdate = false;

	void PreRenderFrame(int idt) override
	{
		if (idt == 0 || idt < m_prevIdt || skippedLastMinimapUpdate)
		{
			...
			skippedLastMinimapUpdate = false;
		}
		else
		{
			skippedLastMinimapUpdate = true;
		}
		m_prevIdt = idt;
...
%endif
	...
	void HandicapCalculate(PlayerRecord& record)
	{
		...
		if (runDiff > 0)
			...
			record.handicap = min(1.0f, record.handicap + 1.0f / (record.statistics.GetStatInt("total-runs")>0?record.statistics.GetStatInt("total-runs"):1));
	...
	void LoadPlayer(SValue& data, PlayerRecord& player) override
	{
		...
		*/
		...
		{
			auto arrKeys = dicUpgrades.GetDictionaryKeys();
...
%endif
	...
	bool TogglePlayerMenu(bool showMenu) override
	{
		ToggleUserWindow(m_playerMenu);
		m_playerMenu.m_pause = showMenu;
		Tutorial::RegisterAction("player_menu");
		return true;
	}

	bool ToggleGuildHall(bool showMenu) override
	{
		ToggleUserWindow(m_guildHallMenu);
		m_guildHallMenu.m_pause = showMenu;
		Tutorial::RegisterAction("guild_menu");
		return true;
	}
}
