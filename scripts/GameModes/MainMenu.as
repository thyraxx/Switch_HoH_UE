...
class MainMenu : BaseGameMode
{
	...
	bool m_ignoreInviteData;

	int m_frameCount;
	bool m_lostConnection;
	bool m_lostConnectionHost;
...
%endif
	...
	void SetMenuStyle(string style)
	{
		...
		if (style == "")
		{
			...
			style = arrAuto[randi(arrAuto.length())].GetString();
		}
		else if (style == "?")
		{
			auto arrKeys = svdicStyles.GetDictionaryKeys();
	...
	void ShowMessage(MenuMessage message)
	{
		...
		else if (message == MenuMessage::LostConnectionHost)
			m_lostConnectionHost = true;
		else if (message == MenuMessage::InviteInSesion)
			m_inviteInSession = true;
		else if (message == MenuMessage::JoinInSesion)
			m_joinInSession = true;
		else if (message == MenuMessage::LobbyJoinError)
...
%endif
	...
	void SetMainMenuBlit()
	{
		...
		m_mainMenu.m_menus.removeRange(1, m_mainMenu.m_menus.length() - 1);
		m_mainMenu.m_menus[0].Show();
...
%PROFILE_STOP
		...
		if (m_lostConnectionHost)
		{
			m_lostConnectionHost = false;
			ShowDialog("connectionlost", "Connection lost. Other players will not be able to join your game unless you create the lobby again.", Resources::GetString(".menu.ok"), null);
		}

		if (m_lobbyJoinError)
		{
			m_lobbyJoinError = false;
			ShowDialog("lobbyjoinerror", "Error joining the lobby.", Resources::GetString(".menu.ok"), null);
		}
		
		if (m_inviteInSession)
		{
			m_inviteInSession = false;
			ShowDialog("inviteinsession", "You have accepted a game invite. Want to proceed and close the current game?", Resources::GetString(".misc.yes"), Resources::GetString(".misc.no"), m_gameMenu.GetCurrentMenu());
		}
		
		if (m_joinInSession)
		{
			m_joinInSession = false;
			ShowDialog("inviteinsession", "You have joined an online game. Want to proceed and close the current game?", Resources::GetString(".misc.yes"), Resources::GetString(".misc.no"), m_gameMenu.GetCurrentMenu());
		}
...
%if TARGET_XB1
		if(m_state == MenuState::MainMenu && m_gamertag !is null && m_strGamertag != "")
		{
			sb.DrawString(vec2( (g_gameMode.m_wndWidth - m_gamertag.GetWidth()) * 0.095, (g_gameMode.m_wndHeight - m_gamertag.GetHeight()) * 0.05), m_gamertag);
		}
%endif
		sb.PopTransformation();
	}
...
//		}
		...
		//Blit - ignore room metadata. Can't know it. //TODO: send it though metadata info
		m_inviteAcceptID = id;
		m_inviteAcceptMaxLevel = Lobby::GetInviteMeta(0);
		if(m_inviteAcceptMaxLevel == 255) m_inviteAcceptMaxLevel = -1;
		m_inviteAcceptMinLevel = Lobby::GetInviteMeta(1);
		m_inviteAcceptMercenary = Lobby::GetInviteMeta(2) == 0 ? false : true;
		m_ignoreInviteData = Lobby::GetInviteMeta(3) == 0 ? true : false;
		
		//print("invite meta: " + m_inviteAcceptMaxLevel + " || " + m_inviteAcceptMinLevel + " || " + m_inviteAcceptMercenary + " || " + m_ignoreInviteData);
	...
	void SystemMessage(string name, SValue@ data)
	{
		...
		else if (name == "SetNGP" && Network::IsServer())
		{
			...
			Lobby::SendBlitLobbyMessage(3, data.GetInteger()); //value (3) hardcoded - BlitMsg::NGP
		}
		else if (name == "SetDownscaling" && Network::IsServer())
		{
			Lobby::SetLobbyData("downscaling", data.GetBoolean() ? "true" : "false");
			Lobby::SendBlitLobbyMessage(4, data.GetBoolean() ? 1 : 0); //value (4) hardcoded - BlitMsg::dOWNSCALING
		}
	}
	...
	void ToggleCharacterSel(string context) override
	{
