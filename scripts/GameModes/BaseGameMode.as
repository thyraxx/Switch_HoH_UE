...
int g_enemyActiveUpdateFreq = 2;
bool g_cullGibs = true;
%endif
...
class BaseGameMode : AGameMode
{
	...
	bool m_showMpMana;
	bool m_analogPreviousUp = true;

	bool m_checkKeyboardResult = false;
	...
	BaseGameMode(Scene@ scene)
	{
		...
		AddVar("g_laser_sight", 1); //Blit default value
		...
		AddVar("ui_minimap_size", 100); //Blit default value
...
%endif
	...
	void UpdatePausedFrame(int ms, GameInput& gameInput, MenuInput& menuInput)
	{
		...
		acc += (float(ms)/1000)*9.0;
		if(acc > 2*PI) acc = acc - 2*PI;
	}
	
	int m_activeEnemiesMPrev;
	bool SetActiveEnemiesMPrev(int v)
	{
		if (m_activeEnemiesMPrev != v)
		{
			m_activeEnemiesMPrev = v;
			return true;
		}
	...
	int m_unitsAmountState = 0;
	bool CheckEnemyAmountState(int activeUnits, int amountState)
	{
		bool ret = false;
		if (m_unitsAmountState >= amountState)
			ret = activeUnits > int(amountState * 0.66f);
			
		ret = ret || activeUnits > amountState;
		if (ret)
		{
			if (m_unitsAmountState != amountState)
				print("AmountState: " + amountState);
				
			m_unitsAmountState = amountState;
			return true;
		}
...
//%if CONSOLE
//		int activeUnits = GetCounterInt("actors_updated");
//
////		if (CheckEnemyAmountState(activeUnits, 300))
////		{
////			g_enemyIdleUpdateFreq = 250;
////			g_enemyActiveUpdateFreq = 250;
////			g_cullGibs = true;
////		}
////		else if (CheckEnemyAmountState(activeUnits, 200))
////		{	
////			g_enemyIdleUpdateFreq = 150;
////			g_enemyActiveUpdateFreq = 180;
////			g_cullGibs = true;
////		}
////		else if (CheckEnemyAmountState(activeUnits, 100))
////		{	
////			g_enemyIdleUpdateFreq = 120;
////			g_enemyActiveUpdateFreq = 90;
////			g_cullGibs = true;
////		}
////		else if (CheckEnemyAmountState(activeUnits, 50))
////		{
////			g_enemyIdleUpdateFreq = 120;
////			g_enemyActiveUpdateFreq = 60;
////			g_cullGibs = true;
////		}
////		else if(CheckEnemyAmountState(activeUnits, 0))
////		{
////			g_enemyIdleUpdateFreq = 120;
////			g_enemyActiveUpdateFreq = 1;
////			g_cullGibs = false;
////		}
//
//
//////////////////
//		if (CheckEnemyAmountState(activeUnits, 110))
//		{	
//			g_enemyIdleUpdateFreq = 250;
//			g_enemyActiveUpdateFreq = 250;
//			g_cullGibs = true;
//		}
//		else if (CheckEnemyAmountState(activeUnits, 85))
//		{	
//			g_enemyIdleUpdateFreq = 200;
//			g_enemyActiveUpdateFreq = 150;
//			g_cullGibs = true;
//		}
//		else if (CheckEnemyAmountState(activeUnits, 60))
//		{	
//			g_enemyIdleUpdateFreq = 200;
//			g_enemyActiveUpdateFreq = 100;
//			g_cullGibs = true;
//		}
//		else if (CheckEnemyAmountState(activeUnits, 30))
//		{
//			g_enemyIdleUpdateFreq = 120;
//			g_enemyActiveUpdateFreq = 60;
//			g_cullGibs = true;
...
%PROFILE_STOP
	...
	Widget@ GetDefaultFocus(Widget@ w)
	{
		if(w.m_defaultFocus && w.m_canFocus && !w.m_invalidated) return w;
		if(w.m_invalidated) return null;
		for(uint i = 0; i < w.m_children.length(); i++)
		{	
			Widget@ ww = GetDefaultFocus(w.m_children[i]);
			if(ww !is null && !ww.m_invalidated) return ww;
		}
		return null;
	}
	...
	void UpdateWidgets(int ms, GameInput& gameInput, MenuInput& menuInput)
	{
		...
		if(m_checkKeyboardResult)
		{
			if(Platform::SystemKeyboardCheckResultReady())
			{
				auto iw = cast<TextInputWidget>(m_widgetInputFocus);
				string res = Platform::SystemKeyboardResult();
				if(res != "") 
				{
%if !TARGET_NX
					if(res.length() > 16)
					{
						ShowDialog("characterlimit",
						 "You have used more than 16 characters, only the first 16 will be used.",
...
%endif
				...
				@m_widgetInputFocus = null;
				m_checkKeyboardResult = false;
			}
			else
			{
				return; // waiting for keyboard result
			}
		}
		...
		//print("UnderCursorValid");
		//print(m_widgetUnderCursor is null);
		if (underCursorValid)
		{
			bool inRoots = false;
			for (uint i = 0; i < m_widgetRoots.length(); i++)
		...
		if (gameInput.Potion.Pressed && (m_tooltip.m_text !=null || !m_tooltip.m_toggled) ) m_tooltip.SetToggle(!m_tooltip.m_toggled);
		...
		if (menuInput.Forward.Released)
		{
			...
			else if (m_widgetUnderCursor !is null && m_widgetUnderCursor.m_canFocusInput)
			{
				...
				if (iw !is null)
				{
					// iw.SetText(Platform::SystemKeyboard(iw.m_text));
					// @m_widgetInputFocus = null;
					Platform::SystemKeyboard(iw.m_text);
					m_checkKeyboardResult = true;
				}
			}
		}
		...
		if (m_widgetUnderCursor !is null && (!m_widgetUnderCursor.m_visible))
			@m_widgetUnderCursor = null;

		vec2 mdir = gameInput.MoveDir;

		bool analogUp = (mdir.x == 0 && mdir.y == 0);
		...
		if (m_widgetUnderCursor !is null)
		{
			...
			if(m_widgetUnderCursor !is null)
			{
				if (menuInput.Up.Down || (mdir.y < -abs(mdir.x)))
					if(timeInput == 0)
					{
						changedFocus = w.FocusTowards(WidgetDirection::Up);
						timeInput += timeInput>=waitInput? waitInput: timeInput+ms;
					}
					else
					{
						if(timeInput >= waitInput)
							...
							{
								timeInterval = timeInterval>=waitInterval? waitInterval: timeInterval+ms;
							}
						else
						{
							timeInput += timeInput>=waitInput? waitInput: timeInput+ms;
				...
				else if (menuInput.Down.Down || (mdir.y > abs(mdir.x)))
					if(timeInput == 0)
					{
				...
				if (menuInput.Left.Down || (mdir.x < -abs(mdir.y)))
					if(timeInput == 0)
					{
				...
				else if (menuInput.Right.Down || (mdir.x > abs(mdir.y)))
					if(timeInput == 0)
					{
				...
				if (changedFocus)
				{
					...
					while (m_widgetUnderCursor.m_host.m_queueMouseLeave.length() > 0)
					{
						auto evt = m_widgetUnderCursor.m_host.m_queueMouseLeave[0];
						m_widgetUnderCursor.m_host.m_queueMouseLeave.removeAt(0);
						evt.m_widget.OnMouseLeave(evt.m_pos);
					}

					
					while (m_widgetUnderCursor.m_host.m_queueMouseEnter.length() > 0)
					{
						auto evt = m_widgetUnderCursor.m_host.m_queueMouseEnter[0];
						m_widgetUnderCursor.m_host.m_queueMouseEnter.removeAt(0);
						evt.m_widget.OnMouseEnter(evt.m_pos, evt.m_force);
					}
				}
			}
		}
		else if (menuInput.Up.Pressed || menuInput.Down.Pressed || menuInput.Left.Pressed || menuInput.Right.Pressed || (!analogUp && m_analogPreviousUp) )
		{
			for (int i = m_widgetRoots.length() - 1; i >= 0; i--)
			{
		...
		if (m_widgetUnderCursor is null && m_widgetRoots.length() > 0)
		{
			
			auto w = GetDefaultFocus(m_widgetRoots[m_widgetRoots.length() - 1].m_widget);
			if (w !is null)
			{
				w.SetHovering(true, w.GetCenter(), true);
			}
		...
		if (m_widgetUnderCursor is null && m_widgetRoots.length() > 0)
		{
			auto w = FirstFocusable(m_widgetRoots[m_widgetRoots.length() - 1].m_widget);
			if (w !is null)
			{
				w.SetHovering(true, w.GetCenter(), true);
			}
		...
		if (menuInput.Up.Released || menuInput.Down.Released || menuInput.Left.Released || menuInput.Right.Released || (!m_analogPreviousUp && analogUp)){timeInput = 0; timeInterval = 0;}
		...
		//print("Inspector");
		//print(m_widgetUnderCursor is null);
		m_analogPreviousUp = analogUp;
	}

	Widget@ FirstFocusable(Widget@ w)
	{
		if (w.m_canFocus && !w.m_invalidated) //Cambiado el 11/01/2019, agregada la condicion de visibilidad, El 16 cambiado a check de validado
			return w;

		for (uint i = 0; i < w.m_children.length(); i++)
		{
			auto ww = w.m_children[i];
			if(ww.m_invalidated) continue;
			if (ww.m_canFocus) //Cambiado el 11/01/2019, agregada la condicion de visibilidad, El 16 cambiado a check de validado
				return ww;
...
%PROFILE_STOP
		...
		*/
		...
		if(m_widgetUnderCursor !is null && m_widgetUnderCursor.m_host.m_queueMouseEnter.length() == 0)
		{
			auto gm = cast<BaseGameMode>(g_gameMode);
			Rect@ rect = m_widgetUnderCursor.GetRectangle(); 
			if(m_widgetUnderCursor.m_parent !is null && cast<ScrollableWidget>(m_widgetUnderCursor.m_parent) !is null)
			{
				rect.top = max(m_widgetUnderCursor.m_origin.y, m_widgetUnderCursor.m_parent.m_origin.y);
				rect.bottom = min(m_widgetUnderCursor.m_origin.y + m_widgetUnderCursor.m_height, m_widgetUnderCursor.m_parent.m_origin.y + m_widgetUnderCursor.m_parent.m_height);
			}
			gm.m_tooltip.Draw(sb,rect);
		}
...
/*
		...
		{
			float anvalue = lerp(0.5, 1.0,((sin(acc)+1)/2));
			//print(anvalue);
...
%endif
	...
	*/
	...
	//"Blit"
	void ToggleCharacterSel(string context)
	{
		return;
	}

	bool TogglePlayerMenu(bool showMenu)
	{
		//print("Base");
		return false;
	}
