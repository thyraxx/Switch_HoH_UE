...
const int g_maxChars = 30; //BLIT: maximum numbers of characters
const int g_maxMercs = 100; //BLIT: maximum numbers of dead mercenaries
...
EffectParams@ LoadEffectParams(UnitPtr unit, SValue& params)
{
	...
	{
%PROFILE_START LoadEffectParams
		...
		if (effectParams !is null)
		{
			...
			for (uint i = 0; i < epKeys.length(); i++)
			{
				...
				if (d !is null && d.GetType() == SValueType::Float)
...
bool g_hasSunderingStrikess = false; //hack to track Sundering Strikes...
bool g_hasFortuitousEvents = false; //hack to track Fortuitous Events...