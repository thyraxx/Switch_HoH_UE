namespace Stats
{
	class StatList
	{
		...
		void AddStat(Stat@ stat)
		{
			...
			m_statsLookup.set(stat.m_name, @stat);
		}
		...
		void CombineStatsFrom(StatList@ other)
		{
			for (uint i = 0; i < other.m_stats.length(); i++)
			{
				...
				auto stat = GetStat(otherStat.m_name);
				if (stat is null)
					continue;
		...
		{
			Stat@ stat;
			m_statsLookup.get(name, @stat);
			return stat;
		}
/*
		Stat@ GetStat(uint hash)
		{
			uint sl = m_stats.length();
			for (uint i = 0; i < sl; i++)
			{
...
*/
		...
		int GetReputationPoints()
		{
			...
			for (uint i = 0; i < sl; i++)
			{
				ret += m_stats[i].m_accomplishedReputation;
		...
		void Load(SValue@ dictStatistics)
		{
			...
			auto keys = dictStatistics.GetDictionaryKeys();
