...
%endif
	...
	void Update(int dt, PlayerRecord@ record)
	{
		...
		if(GetVarBool("ui_hud_visibility") == false)
		{
			m_wTopbar.m_visible = false;
			m_wHealthGui.m_visible = false;
			m_wSkillGui.m_visible = false;
			m_wBuffList.m_visible = false;
			m_wBossBars.m_visible = false;
			if(!uiHidden)
			{
				uiHidden = true;
				minimapVal = GetVarInt("ui_minimap_size");
		...
		{
			uiHidden = false;
			SetVar("ui_minimap_size", minimapVal);
		}
...
%endif
		...
		if (player !is null)
		{
			...
			if (localPlayer !is null)
			{
				extraStats = localPlayer.m_cachedStatsAdd;
