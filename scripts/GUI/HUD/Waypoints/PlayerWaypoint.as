class PlayerWaypoint : Waypoint
{
	...
	float fadeEndX = (g_gameMode.m_wndWidth/2.0f) - 50.0f;
	float fadeStartX = (g_gameMode.m_wndWidth/2.0f);
	...
	vec4 GetColor() override
	{
		...
		vec2 distance = vec2(abs(m_pos.x - m_fromPos.x), abs(m_pos.y - m_fromPos.y));
		float a = 1.0f;
		...
		*/
		...
		else if(distance.y < fadeStartY && distance.x < fadeStartX)
		{
			float ax = 0.0f;
			float ay = 0.0f;

			ax = 1.0f - (fadeStartX - distance.x)/( fadeStartX - fadeEndX);
			
			ay = 1.0f - (fadeStartY - distance.y)/( fadeStartY - fadeEndY);
