...
//		print(m_focusEffect);
	...
	void DoDraw(SpriteBatch& sb, vec2 pos) override
	{
		...
		{
			m_bitmapText.SetColor(Tweak::ScaleButtonTextHoverColor);
		}
		else
		{
			m_bitmapText.SetColor(m_color);
		...
		if(m_bitmapText.GetWidth() > m_width and m_canScroll)
		{
			spacingOffset = m_vertTextSpacing*2;
			textPos.x += m_drawOffset - GetTextOffset().x + spacingOffset;
			Scroll(textPos);
		}

		sb.DrawString(textPos + vec2(-m_scrollOffset, 0), m_bitmapText);
	...
	{
		auto limit = textPos.x + m_bitmapText.GetWidth() - (m_origin.x + m_width) + m_vertTextSpacing;
