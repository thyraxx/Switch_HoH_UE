class ScrollableWidget : ClipWidget
{
	...
	bool m_scrollUpThroughWidget = false;
	bool m_scrollDownThroughWidget = false;

	Widget@ lastWidgetInCursor = null;
	...
	{
		ClipWidget::Update(dt);
		if(g_gameMode.m_widgetUnderCursor !is null && IsChild(g_gameMode.m_widgetUnderCursor))
		{	
			vec2 o = g_gameMode.m_widgetUnderCursor.m_origin;
			int h = g_gameMode.m_widgetUnderCursor.m_height;
			m_scrollUpThroughWidget = o.y < m_origin.y && h > m_height;
			m_scrollDownThroughWidget = o.y + float(h) > m_origin.y + float(m_height) && h > m_height;
		}
	}
	...
	bool IsChild(Widget@ w)
	{
		for(uint i = 0; i < m_children.length(); i++)
		{
			if(w is m_children[i])return true;
		}
		return false;
	}
	...
	bool FocusTowards(WidgetDirection dir, Widget@ originWidget = null) override
	{
		...
		if ((dir == WidgetDirection::Up || dir == WidgetDirection::Down) && m_autoScroll)
		{
			...
			if (m_scrollUpThroughWidget || m_scrollDownThroughWidget)
			{
				if (SpaceToScroll())
				{
					if (dir == WidgetDirection::Up && m_scrollUpThroughWidget)
						m_autoScrollValue -= m_autoScrollArrowDelta;
					else if (dir == WidgetDirection::Down && m_scrollDownThroughWidget)
						m_autoScrollValue += m_autoScrollArrowDelta;
				}
		...
		return ClipWidget::FocusTowards(dir, originWidget);
	}
