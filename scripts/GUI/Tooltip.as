...
*/
	...
	void Draw(SpriteBatch& sb, Rect@ rect)
	{
		...
		vec2 size = GetSize();
		vec2 pos = vec2(rect.right, rect.top + rect.GetHeight()/2);
		vec2 offset = vec2(2,0);//vec2();

		bool inTop = true;

		if(rect.GetCenter().y > g_gameMode.m_wndHeight/2 + 20)
		{
			offset.y = -offset.y - size.y;
			inTop = false;
		}

		if(rect.GetCenter().x > g_gameMode.m_wndWidth/2 + 20)
		{
			offset.x = -offset.x - size.x - rect.GetWidth();
		}
...
/*
		if (pos.x + offset.x + size.x > g_gameMode.m_wndWidth)
			offset.x = -offset.x - size.x;
		if (pos.y + offset.y + size.y > g_gameMode.m_wndHeight)
			offset.y = -offset.y - size.y; //+ rect.GetHeight()/2;
*/
		pos += offset;


/*
		if(pos.x < rect.right && pos.x + size.x > rect.left && pos.y < rect.bottom && pos.y + size.y > rect.top)
		{
			if(rect.left - size.x > 0)
			{
				pos.x = rect.left - size.x;
			}
		}
		if(pos.x < rect.right && pos.x + size.x > rect.left && pos.y < rect.bottom && pos.y + size.y > rect.top)
		{
			if(rect.bottom + size.y < g_gameMode.m_wndHeight)
			{
				pos.y = rect.bottom;
			}
		}
		if(pos.x < rect.right && pos.x + size.x > rect.left && pos.y < rect.bottom && pos.y + size.y > rect.top)
		{
			if(rect.top - size.y > 0)
			{
				pos.y = rect.top - size.y;
			}
		}
*/
		...
		if (pos.y < 0 || pos.y + size.y > g_gameMode.m_wndHeight)
		{
			pos.y = (g_gameMode.m_wndHeight-size.y)/2;
		}

		if( (pos.x > rect.left && pos.x < rect.right) || (pos.x + size.x > rect.left && pos.x + size.x < rect.right))
		{
			if(inTop)
			{
				pos.y = rect.bottom;
