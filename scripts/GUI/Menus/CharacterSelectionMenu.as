namespace Menu
{
	class CharacterSelectionMenu : HwrMenu
	{
		...
		void Update(int dt) override
		{
			...
			if(g_gameMode.m_widgetUnderCursor !is cast<Widget>(m_wList.GetChecked()))
			{
				m_wList.Toggled(cast<ICheckableWidget>(g_gameMode.m_widgetUnderCursor));
			}
			if(m_provider.m_menus[m_provider.m_menus.length()-1] is this && (g_gameMode.m_dialogWindow is null || !g_gameMode.m_dialogWindow.m_visible))
			{
				if(gi.Potion.Pressed)// CHECK IF CONTROLS CHANGE ///////////////////////////////////////////////////////////
				{
					int numchars = HwrSaves::GetNumActiveCharacters();
					if(numchars >= g_maxChars)
						g_gameMode.ShowDialog("toomanycharacters", "Too many characters created. Delete one and try again.", Resources::GetString(".menu.ok"), null);
					else
						OpenMenu(CharacterCreationMenu(m_provider, m_context), "gui/main_menu/character_creation.gui");
				}
				else if(gi.MapOverlay.Pressed)// CHECK IF CONTROLS CHANGE ///////////////////////////////////////////////////////////// CHECK IF CONTROLS CHANGE ///////////////////////////////////////////////////////////
				{
					auto wChecked = cast<Widget>(m_wList.GetChecked());
					if (wChecked is null)
						return;
					selectedCharacter = parseInt(m_wList.GetChecked().GetValue());
				...
				else if(gi.Use.Pressed)// CHECK IF CONTROLS CHANGE ///////////////////////////////////////////////////////////
				{
					auto wChecked = m_wList.GetChecked();
					if (wChecked is null)
						return;
					auto wCwidget = cast<Widget>(wChecked);
					if(wCwidget !is null && !wCwidget.m_enabled)
						return;
		...
		void OnFunc(Widget@ sender, string name) override
		{
			...
			else if (name == "delete yes")
			{
				if (selectedCharacter == -1)
					return;
				HwrSaves::DeleteCharacter(selectedCharacter);
				selectedCharacter = -1;
				ReloadList();
			}
			else
