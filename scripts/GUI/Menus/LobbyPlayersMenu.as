namespace Menu
{
	class LobbyPlayersMenu : Menu
	{
		...
		void Initialize(GUIDef@ def) override
		{
			...
			@m_wInviteHint = m_widget.GetWidgetById("invite-hint");
		...
		void UpdatePlayerList()
		{
			...
			int numPlayers = HwrSaves::PeersWithSave();
...
%endif
		...
		void Update(int dt) override
		{
			...
			auto input = GetInput();
			string joinable = Lobby::GetLobbyData("joinable");
			string online = Lobby::GetLobbyData("online");
			if(joinable == "true" && online == "1")
			{
				if(input.MapOverlay.Pressed)
				{
						Lobby::InviteFriend();
				}
				if(!m_wInviteHint.m_visible)
				{
					m_wInviteHint.m_visible = true;
					m_wInviteHint.m_parent.m_width += (m_wInviteHint.m_width + 6);
				}
			}
			else
			{
