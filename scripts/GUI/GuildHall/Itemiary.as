	...
	void AddItem(ItemiaryEntry@ entry)
	{
		...
		if (wNameContainer !is null)
		{
			...
			if (wName !is null)
			{
				...
				wNewItem.m_tooltipTitle = "\\c" + GetItemQualityColorString(item.quality) + utf8string(Resources::GetString(item.name)).toUpper().plain();
				wNewItem.m_tooltipText = Resources::GetString(item.desc);

				if (item.set !is null)
				{
					wNewItem.m_tooltipText += "\n\n";
					wNewItem.m_tooltipText += GetItemSetColorString(GetLocalPlayerRecord(), item);
				}
