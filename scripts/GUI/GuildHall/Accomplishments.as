class GuildHallAccomplishmentsTab : GuildHallMenuTab
{
	...
	void RefreshList()
	{
		...
		for (uint i = 0; i < stats.m_stats.length(); i++)
		{
			...
			if (!hidden)
			{
				wNewBar.m_tooltipEnabled = true;
				wNewBar.m_tooltipTitle = accomplishmentName;
				wNewBar.m_tooltipText = lastAccomplishment.GetDescription();
				wNewBar.AddTooltipSub(m_def.GetSprite("reputation-points"), formatThousands(lastAccomplishment.m_reputation));
			}
