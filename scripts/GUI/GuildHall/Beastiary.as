	...
	void AddCategory(Widget@ wList, string name, array<BestiaryEntry@>@ units)
	{
		...
		for (uint i = 0; i < units.length(); i++)
		{
			...
			if (wName !is null)
			{
				...
				wNewItem.m_tooltipTitle = Resources::GetString(unitName);
				if (unitDescription != "")
					wNewItem.m_tooltipText = Resources::GetString(unitDescription) + "\n\n";
				wNewItem.m_tooltipText += Resources::GetString(".bestiary.expreward") + ": " + formatThousands(expReward);

				wNewItem.AddTooltipSub(m_spriteHealth, formatThousands(hp));

				if (armor > 0)
					wNewItem.AddTooltipSub(m_spriteArmor, formatThousands(armor));

				if (resistance > 0)
					wNewItem.AddTooltipSub(m_spriteResistance, formatThousands(resistance));
			}
