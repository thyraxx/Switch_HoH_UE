class CustomizationDyesTab : MenuTab
{
	...
	bool BackDyes()
	{
		if(g_gameMode.m_widgetUnderCursor.m_parent is m_wListColors)
		{
			for(int i = 0; i < m_wListMaterial.m_children.length(); i++)
			{
				auto wcheck = cast<CheckBoxWidget>(m_wListMaterial.m_children[i]);
				if(wcheck !is null && wcheck.m_checked)
				{
					wcheck.SetHovering(true, wcheck.GetCenter());
					break;
				}
			}
			return true;
	...
	{
		auto dyeMap = Materials::GetDyeMapping(m_base.m_charClass);
		bool selectedColor = g_gameMode.m_widgetUnderCursor !is null && g_gameMode.m_widgetUnderCursor.m_parent is m_wListMaterial;
		// Load material list
		...
		for (uint i = 0; i < dyeMap.m_categories.length(); i++)
		{
			...
			if(m_dyeSelecting == i && selectedColor) wNewMaterial.SetHovering(true, wNewMaterial.GetCenter());
		}
		...
		selectedColor = g_gameMode.m_widgetUnderCursor !is null && g_gameMode.m_widgetUnderCursor.m_parent is m_wListColors;
		...
		for (uint i = 0; i < ownedDyes.length(); i++)
		{
			...
			if(wNewColor.m_checked && selectedColor) wNewColor.SetHovering(true, wNewColor.GetCenter());
		}
	...
	bool OnFunc(Widget@ sender, string name) override
	{
		...
		if (parse[0] == "change-material")
		{
			...
			OnShow();
			for(int i = 0; i < m_wListColors.m_children.length(); i++)
			{
				auto wcheck = cast<CheckBoxWidget>(m_wListColors.m_children[i]);
				if(wcheck.m_checked)
				{
					wcheck.SetHovering(true, wcheck.GetCenter());
				}
