...
class MercenaryTrainerNPC : ScriptWidgetHost
{
	...
	void ReloadList()
	{
		...
		{
			auto wButton = cast<ShopButtonWidget>(g_gameMode.m_widgetUnderCursor.GetWidgetById("button"));
			if(wButton !is null)
			{
				func = wButton.m_func;
			}
		...
		for (uint i = 0; i < MercenaryUpgradeDef::Instances.length(); i++)
		{
			...
			if(wButton !is null && wButton.m_func == func) wButton.SetHovering(true, wButton.GetCenter());
		}
