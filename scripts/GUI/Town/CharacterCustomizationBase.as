class CharacterCustomizationBase
{
	...
	void Initialize(GUIDef@ def, PlayerRecord@ record = null)
	{
		...
		array<string> names = {
			...
			"frank", "scott", "eric", "stephen", "andrew", "raymond", "gregory", "joshua", "jerry", "dennis", "walter", "oscar", "patrick", "peter", "harold", "douglas", "henry", "carl",
			"arthur", "ryan", "roger", "victor"
		};
	...
	//TODO merge
	/*
			auto wList = cast<ScrollableWidget>(m_wSelection.GetWidgetById("list-colors"));
			if(wList.GetChildrenPosition(g_gameMode.m_widgetUnderCursor) != -1)
			{
				m_categoryWidget.SetHovering(true, m_categoryWidget.GetCenter());
				return false;
			}
			else
	*/		
			auto dyeTab = cast<CustomizationDyesTab>(m_tabSystem.GetTab("dyes")); 
			
			if(dyeTab is null || !dyeTab.BackDyes())
			{
				g_gameMode.ShowDialog("charcustomback",
					"Go back?",
