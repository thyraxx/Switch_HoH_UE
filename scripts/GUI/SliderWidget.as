...
class SliderWidget : Widget
{
	...
	void Update(int dt) override
	{
		...
		GameInput@ ginput = GetInput();

		if(m_hovering && (input.Left.Down || input.Right.Down || ginput.MoveDir.x != 0)) 
		{
			m_timePressed += dt;
		}
		else if(m_timePressed != 0)
		{
			m_timePressed = 0;	
		}
	...
	bool FocusTowards(WidgetDirection dir, Widget@ originWidget) override
	{

		//float movement = (m_timePressed>2500)? 0.025 : ( (m_timePressed>1000) ? 0.005 : min(0.005, (1.0f / (m_max - m_min)) ));
		

		float movement = (m_timePressed>1500)?(max(m_epsilon*4,(m_max - m_min)/100)):m_epsilon;
		if(m_labelType == SliderLabelType::RealValue)
		{
			float value = GetScaleValue();
			float realValue = lerp(m_min, m_max, value);
			movement = (m_timePressed>1500)?max(0.025, (1.0f / (m_max - m_min))): (1.0f / (m_max - m_min));
		}
		switch (dir)
		{
			case WidgetDirection::Left: 
				...
				UpdateVar(); 
				return false;
			case WidgetDirection::Right:m_value += movement;
				if (m_value > 1.0f)
					m_value = 1.0f;
				else if (m_value < 0.0f)
					m_value = 0.0f;
