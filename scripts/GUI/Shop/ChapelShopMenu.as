class ChapelShopMenuContent : ShopMenuContent
{
	...
	Upgrades::UpgradeStep@ m_upgradeStepPurchased = null;
		...
		for (uint i = 0; i < m_shop.m_rows.length(); i++)
		{
			...
			for (uint j = 0; j < row.length(); j++)
			{
				...
				if (wNewItem !is null)
				{
					...
					if(m_upgradeStepPurchased is step)
					{
						print(step.GetTooltipTitle());
					 	wNewItem.SetHovering(true, wNewItem.GetCenter());
					}
				}
			}
		}
	...
	void OnFunc(Widget@ sender, string name) override
	{
		...
		else if (name == "buy-item")
		{
			...
			@m_upgradeStepPurchased = btn.m_upgradeStep;
			print(m_upgradeStepPurchased.GetTooltipTitle());
