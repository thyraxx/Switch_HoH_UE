class FountainShopMenuContent : ShopMenuContent
{
	...
	int m_lastPresetDeleted = -1;

	string m_funcCurrentWidget = "";
	...
	void ReloadPresets()
	{
		...
		if(m_lastPresetDeleted > town.m_fountainPresets.length() - 1) m_lastPresetDeleted = town.m_fountainPresets.length() - 1;
		...
		for (uint i = 0; i < town.m_fountainPresets.length(); i++)
		{
			...
			if(wButtonLoad.m_func == m_funcCurrentWidget) wButtonLoad.SetHovering(true, wButtonLoad.GetCenter());
			if(wButtonSave.m_func == m_funcCurrentWidget) wButtonSave.SetHovering(true, wButtonSave.GetCenter());
		}
	}
...
%endif
		else if (parse[0] == "save-preset")
		{
			...
			{
				//auto ButtonWidget = cast<ScalableSpriteButtonWidget>(g_gameMode.m_widgetUnderCursor);
				//if(ButtonWidget !is null) funcCurrentWidget = ButtonWidget.m_func;	
				m_funcCurrentWidget = name;	
			}
			if(gm.m_townLocal.m_fountainPresets[index].effects.length() > 0)
			{
				gm.ShowDialog("override-preset " + parse[1],
						 "Do you want to override Preset "+ (parseUInt(parse[1])+1) +" ?",
						  Resources::GetString(".misc.yes"), Resources::GetString(".misc.no"), m_shopMenu);
				return;
			}
		...
		else if(parse[0] == "override-preset")
		{
			if(parse[2] != "yes") return;
			uint index = parseUInt(parse[1]);
