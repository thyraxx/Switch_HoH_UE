class ScalableSpriteButtonWidget : ICheckableWidget, Widget
{
	...
	array<Sprite@> m_spriteHoverDown;
	...
	void Load(WidgetLoadingContext &ctx) override
	{
		...
		if(def.GetSprite(spriteSet + "-hoverdown-left") !is null)
		{
			m_spriteHoverDown.insertLast(def.GetSprite(spriteSet + "-hoverdown-left"));
			m_spriteHoverDown.insertLast(def.GetSprite(spriteSet + "-hoverdown-mid"));
			m_spriteHoverDown.insertLast(def.GetSprite(spriteSet + "-hoverdown-right"));
		}
		else
		{
			m_spriteHoverDown.insertLast(def.GetSprite(spriteSet + "-down-left"));
			m_spriteHoverDown.insertLast(def.GetSprite(spriteSet + "-down-mid"));
			m_spriteHoverDown.insertLast(def.GetSprite(spriteSet + "-down-right"));
		}
		...
		m_canFocus =  ctx.GetBoolean("canfocus", false, true);
	}
	...
	vec4 GetTextColor()
	{
		...
		else if (m_buttonDown || (m_checked && m_checked) || !m_checkable)
	...
	array<Sprite@> GetSprites()
	{
		...
		{
			if((m_buttonDown || (m_checkable && m_checked)) && m_hovering)
				return m_spriteHoverDown;
			else if (m_buttonDown || (m_checkable && m_checked))
				return m_spriteDown;
			else if (m_hovering)
