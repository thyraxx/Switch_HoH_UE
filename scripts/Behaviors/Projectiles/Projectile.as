class ProjectileCached : ProjectileBaseCached
{
	uint m_team;
	int m_ttl;
	float m_liveRangeSq;
	...
	ProjectileCached(UnitPtr unit, SValue& params)
	{
		super(unit, params);
...
class Projectile : ProjectileBase
{
	...
	{
		auto cache = cast<ProjectileCached>(UnitProductionCache::GetCached(unit));
		if (cache is null)
			@cache = cast<ProjectileCached>(UnitProductionCache::AddCached(unit, ProjectileCached(unit, params)));

		super(unit, params, cast<ProjectileBaseCached>(cache));
		
		m_ttl = cache.m_ttl;
		m_liveRangeSq = cache.m_liveRangeSq;
