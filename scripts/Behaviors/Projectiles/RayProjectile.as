class RayProjectileCached : ProjectileBaseCached
{
	uint m_team;
	int m_ttl;
	...
	RayProjectileCached(UnitPtr unit, SValue& params)
	{
		super(unit, params);
...
class RayProjectile : ProjectileBase
{
	...
	{
		auto cache = cast<RayProjectileCached>(UnitProductionCache::GetCached(unit));
		if (cache is null)
			@cache = cast<RayProjectileCached>(UnitProductionCache::AddCached(unit, RayProjectileCached(unit, params)));

		super(unit, params, cast<ProjectileBaseCached>(cache));

		m_ttl = cache.m_ttl;
		m_selfDmg = cache.m_selfDmg;
		m_teamDmg = cache.m_teamDmg;
		m_bounces = cache.m_bounces;
		m_bounceSpeedMul = cache.m_bounceSpeedMul;
		m_penetration = cache.m_penetration;
		m_penetrationIntensityMul = cache.m_penetrationIntensityMul;
		m_penetrateAll = cache.m_penetrateAll;
		m_bounceOnCollide = cache.m_bounceOnCollide;
		@m_anim = cache.m_anim;
		@m_soundBounce = cache.m_soundBounce;
		m_team = cache.m_team;
		
		@m_effects = CopyOrLoadEffects(cache.m_effects, unit, params);
		@m_missEffects = CopyOrLoadEffects(cache.m_missEffects, unit, params, "miss-");
	}

	RayProjectile(UnitPtr unit, SValue& params, RayProjectileCached@ cache)
	{
		super(unit, params, cast<ProjectileBaseCached>(cache));
