...
class ProjectileBaseCached : UnitProductionCache::IBase
{
	string m_fx;
	bool m_penetrating;
	bool m_seeking;
	...
	ProjectileBaseCached(UnitPtr unit, SValue& params)
	{
		...
		m_effectParams = (dat !is null && dat.GetType() == SValueType::Dictionary);
	}
}
...
abstract class ProjectileBase : IProjectile
{
	...
	ProjectileBase(UnitPtr unit, SValue& params)
	{
		...
		auto cache = cast<ProjectileBaseCached>(UnitProductionCache::GetCached(unit));
		if (cache is null)
			@cache = cast<ProjectileBaseCached>(UnitProductionCache::AddCached(unit, ProjectileBaseCached(unit, params)));
	
	
		m_fx = cache.m_fx;
		
		m_penetrating = cache.m_penetrating;
		m_seeking = cache.m_seeking;
		m_seekTurnSpeed = cache.m_seekTurnSpeed;

		m_speed = cache.m_speed;
		m_speedDelta = cache.m_speedDelta;
		m_speedDeltaMax = cache.m_speedDeltaMax;
		
		m_blockable = cache.m_blockable;
		@m_soundHit = cache.m_soundHit;
		@m_soundShoot = cache.m_soundShoot;
		@m_soundLoop = cache.m_soundLoop;

		if (cache.m_effectParams)
			@m_effectParams = LoadEffectParams(unit, params);
	}
	
	ProjectileBase(UnitPtr unit, SValue& params, ProjectileBaseCached@ cache)
	{
		m_unit = unit;
