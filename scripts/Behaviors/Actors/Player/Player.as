...
class Player : PlayerBase
{
	...
	ivec2 m_cachedStatsAdd;
	float m_cachedSpellCostMul;
...
%endif
		...
		{
			//check dpad
			auto menuInput = GetMenuInput();
			if(menuInput.Left.Down)
			{
				moveDir.x = -1;
			}
			if(menuInput.Right.Down)
			{
				moveDir.x = 1;
			}
			if(menuInput.Down.Down)
			{
				moveDir.y = 1;
			}
			if(menuInput.Up.Down)
			{
				moveDir.y = -1;
			}
		...
		//agregado 10/01/2019 
		aimDir = (length(aimDir) == 0) ? vec2(cos(m_dirAngle),sin(m_dirAngle)) : aimDir; 
		//
...
%PROFILE_START Regen
		...
		ivec2 stats = g_allModifiers.StatsAdd(this);
		m_cachedStatsAdd = stats;
		m_cachedSpellCostMul = g_allModifiers.SpellCostMul(this);
...
%PROFILE_START Moving
		...
		auto moveDirLenSq = lengthsq(moveDir);
		
		if (moveDirLenSq < minSpeed * minSpeed)
			moveDir = normalize((moveDirLenSq > 0) ? moveDir : aimDir) * minSpeed;
		...
		bool walking = (lengthsq(moveDir) > 0.1);
