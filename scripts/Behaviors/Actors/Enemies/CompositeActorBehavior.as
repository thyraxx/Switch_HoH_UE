...
class CompositeActorBehaviorCached : UnitProductionCache::IBase
{
	string m_team;
	bool m_countsAsKill;
	...
	bool m_collideWithTeam;
	

	CompositeActorBehaviorCached(UnitPtr unit, SValue& params)
	{
		m_team = GetParamString(unit, params, "team", false, "enemy");
		m_countsAsKill = GetParamBool(unit, params, "counts-as-kill", false, true);
...
%endif
		...
		m_movement = GetParamString(unit, movement, "class");
		...
		m_collideWithTeam = GetParamBool(unit, params, "collide-with-team", false, true);
	}
}
...
%endif
	...
	{
		auto cache = cast<CompositeActorBehaviorCached>(UnitProductionCache::GetCached(unit));
		if (cache is null)
			@cache = cast<CompositeActorBehaviorCached>(UnitProductionCache::AddCached(unit, CompositeActorBehaviorCached(unit, params)));
			
	
		SetTeam(cache.m_team, cache.m_countsAsKill);
		super(unit);

		m_maxHp = cache.m_maxHp;
		m_hp = 1.0;

		m_floatingHurt = cache.m_floatingHurt;
		
		m_impenetrable = cache.m_impenetrable;
		m_expReward = cache.m_expReward;

		m_unitHeight = cache.m_unitHeight;

		m_aggroRange = cache.m_aggroRange;
		m_maxRange = cache.m_maxRange;

		m_canHeal = cache.m_canHeal;
		m_bestiaryOverride = cache.m_bestiaryOverride;
		
		
		@m_aggroSound = cache.m_aggroSound;
		@m_hurtSound = cache.m_hurtSound;
		@m_deathSound = cache.m_deathSound;
		
		@m_lootDef = cache.m_lootDef;
		@m_gore = cache.m_gore;

		m_armor = cache.m_armor;
		m_enemyType = cache.m_enemyType;
		
		m_evadeChance = cache.m_evadeChance;
		m_evadeFx = cache.m_evadeFx;
		

		m_mpScaleFact = cache.m_mpScaleFact;
		m_ngpScale = cache.m_ngpScale;
		m_targetable = cache.m_targetable;
		m_targeting = cache.m_targeting;
		m_mustSeeTarget = cache.m_mustSeeTarget;
		
		m_targetSearchCd = 0;
		
		m_hitFx = cache.m_hitFx;

		m_crosshairColors = cache.m_crosshairColors;

		m_debuffScale = cache.m_debuffScale;
		m_noBuffs = cache.m_noBuffs;
		m_buffsImmune = cache.m_buffsImmune;
		m_buffsImmuneTags = cache.m_buffsImmuneTags;
...
%endif
		...
		{
			SValue@ movement = GetParamDictionary(unit, params, "movement", true);
			@m_movement = cast<ActorMovement>(InstantiateClass(cache.m_movement, unit, movement));
		}
		...
		m_holdAngleOnCast = cache.m_holdAngleOnCast;
		if (cache.m_effectParams)
		{
			SValue@ dat = GetParamDictionary(unit, params, "effect-params", false);
			if (dat !is null && dat.GetType() == SValueType::Dictionary)
			{
				...
				auto epKeys = dat.GetDictionaryKeys();
		...
		m_windScale = cache.m_windScale;
		
		m_unit.SetCollisionTeam(uint16(GetActorListId(Team)));
		m_unit.SetShouldCollideWithTeam(cache.m_collideWithTeam);
...
%if CONSOLE
		if (m_targeting)
			m_unit.SetUpdateFrequency(g_enemyIdleUpdateFreq);
%endif
	}
...
%PROFILE_START CompositeActorBehavior::Damage
...
%if CONSOLE
		if (m_target is null)
			...
			m_unit.SetUpdateFrequency(g_enemyActiveUpdateFreq);
%endif
	}
